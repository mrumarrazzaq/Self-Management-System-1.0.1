...
class ExampleUpdate extends StatefulWidget {
  final String id;

  const ExampleUpdate({required this.id, Key? key}) : super(key: key);
  @override
  _ExampleUpdateState createState() => _ExampleUpdateState();
}

class _ExampleUpdateState extends State<ExampleUpdate> {
  ...
  Future<void> updateEvent(id, taskTitle, taskNote) {
    return events
        .doc(id)
        .update({
        ...
        .then((value) => print('Event Update by email : $userEmail'))
        .catchError((error) => print('Faild to Update Event $error'));
  }
...
//      _addTasktoDb();
      updateEvent(widget.id, taskTitle, taskNote);
